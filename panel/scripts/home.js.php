...
var nvd3Charts = function() {
    ...
    function statBonus() {
        return JSON.parse($.ajax({
            dataType: "text",
            async: false,
            url: 'jsone.php?t=vwbonussummary&w=noid&search=<?=$_SESSION['sjn58']?>&f=f'  
            }).responseText);
    }
...
/*    $('#TableBonus').DataTable({"pageLength": 10});
    $('#TableSponsor').DataTable({"pageLength": 10});
    $('#TablePresenter').DataTable({"pageLength": 10});
    $('#TableDtlBonus').DataTable({"pageLength": 10});*/
} );
</script>